    ...
    if ("gene_biotype" %in% colnames(mcols(annotated_peaks))) {
        if (is.null(biotype_priority)) {
            ...
            biotype_score <- ifelse(
                mcols(annotated_peaks)$gene_biotype == "protein_coding" | 
                     mcols(annotated_peaks)$transcript_biotype == "protein_coding", weight, 0
            )
        } else {
...
#' @param sequencing_method Sequencing method. For PolII, it can be "ChIP-seq", "GRO-seq", "PRO-seq". 
#'  For RBP, it can be "ChIP-seq", "CLIP-seq", "iCLIP", "eCLIP".
#' @return GRanges with priority_score column
#' @export
prioritizeAnnotations <- function(annotated_peaks,
    ...
    switch(factor_type,
        "TF" = prioritizeTFAnnotations(annotated_peaks, max_promoter_distance = 3000L),
        "H3K4me3" = prioritizePromoterHistoneAnnotations(annotated_peaks, "H3K4me3", max_promoter_distance = 3000L),
        "H3K4me2" = prioritizePromoterHistoneAnnotations(annotated_peaks, "H3K4me2", max_promoter_distance = 3000L),
        "H3K27ac" = prioritizeEnhancerHistoneAnnotations(annotated_peaks, "H3K27ac", max_enhancer_distance = 50000L),
        "H3K4me1" = prioritizeEnhancerHistoneAnnotations(annotated_peaks, "H3K4me1", max_enhancer_distance = 50000L),
        ...
        "PolII" = prioritizePolIIAnnotations(annotated_peaks, sequencing_method = sequencing_method),
        "RBP" = prioritizeRBPAnnotations(annotated_peaks, sequencing_method = sequencing_method, use_transcript_level = TRUE),
        "3end" = prioritizeThreeEndAnnotations(annotated_peaks, max_tes_distance = 3000L),
        ...
        "intergenic" = prioritizeIntergenicAnnotations(annotated_peaks, max_intergenic_distance = 50000L),
