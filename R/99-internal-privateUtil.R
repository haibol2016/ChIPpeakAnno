...
getRelationship <- function(queryHits, subjectHits) {
    if (!inherits(queryHits, "GRanges")) {
        stop("'queryHits' must be a GRanges object", call. = FALSE)
    }
    if (!inherits(subjectHits, "GRanges")) {
        stop("'subjectHits' must be a GRanges object", call. = FALSE)
    }
    strand_neg <- strand(subjectHits) == "-"
    FeatureStart <- as.numeric(ifelse(strand_neg, 
                                      end(subjectHits), 
                                      start(subjectHits)))
    FeatureEnd <- as.numeric(ifelse(strand_neg, 
                                    start(subjectHits), 
                                    end(subjectHits)))
    PeakStart <- as.numeric(ifelse(strand_neg, end(queryHits), 
                                   start(queryHits)))
    PeakEnd <- as.numeric(ifelse(strand_neg, start(queryHits), 
                                end(queryHits)))
    ss <- PeakStart - FeatureStart
    ee <- PeakEnd - FeatureEnd
    se <- PeakStart - FeatureEnd
    es <- PeakEnd - FeatureStart
    upstream <- ifelse(strand_neg, es > 0L, es < 0L)
    downstream <- ifelse(strand_neg, se < 0L, se > 0L)
    includeFeature <- ifelse(strand_neg, (ss >= 0L & ee <= 0L), 
                            (ss <= 0L & ee >= 0L))
    overlap <- ss == 0L & ee == 0L
    inside <- ifelse(strand_neg, (ss <= 0L & ee >= 0L), 
                    (ss >= 0L & ee <= 0L))
    overlapStart <- ifelse(strand_neg, (ss > 0L & es <= 0L & ee >= 0L), 
                          (ss < 0L & es >= 0L & ee <= 0L))
    overlapEnd <- ifelse(strand_neg, (ss < 0L & se >= 0L & ee <= 0L), 
                        (ss > 0L & se <= 0L & ee >= 0L))
...
trimPeakList <- function(Peaks, ignore.strand, by, keepMetadata = FALSE) {
    ...
    if (any(duplicated(.gr))) {
        stop("Input contains duplicated ranges. Please recheck your inputs.",
             call. = FALSE)
    }
    if (any(is.null(names(Peaks))) || 
        ...
        message("Duplicated or NA names found. ",
                "Renaming all names by numbers.")
        names(Peaks) <- formatC(seq_along(Peaks), 
                                width = nchar(as.character(length(Peaks))), 
                                flag = "0")
    }
    ...
    if (by == "feature") {
        if (is.null(feature)) {
            stop("Need 'feature' metadata for each input", call. = FALSE)
        }
...
vennCounts <- function(PeaksList, n, names,
    ...
    if (by == "base") {
        ...
        # problem for coverage: can not separate for different source
        for (i in seq_len(n)) {
            names(PeaksList[[i]]) <- 
        ...
        ol_query <- Peaks[queryHits(ol)]
        # ol.subject <- disj[subjectHits(ol)]  # Not used in current implementation
        group <- gsub(paste0(NAME_conn_string, ".*$"), 
                      "", 
                      names(ol_query))
    ...
    if (by == "feature") {
        ...
        if (length(Peaks) < 10000) {
            ...
            olm <- olm[sub(paste0(NAME_conn_string, ".*?$"), "", 
                          names(Peaks[ol[, 1]])) !=
                         sub(paste0(NAME_conn_string, ".*?$"), "", 
                             names(Peaks[ol[, 2]])), , drop = FALSE]
    ...
    NAME_conn_string_wild <- paste0(NAME_conn_string, ".*?$")
    all.df <- data.frame(rnames = unlist(all), 
                    groupID = rep(seq_along(all), vapply(all, length, 
                                                         FUN.VALUE = integer(1))))
...
#                 levels=c(0,1))
            xlist1[[i]] <- xlist[[i]]
        } else { ## increase the efficiency by change list to dataframe
#             xlist1[[i]] <- 
...
#             }))), levels=c(0,1))
    ...
    if (connectedPeaks == "keepAll") {
        ...
        all.m <- lapply(all, 
                        function(.ele) {
                            gsub(NAME_conn_string_wild, "", .ele)
                        })
        all.m <- all.m[vapply(all.m, function(.ele) length(unique(.ele)) > 1, 
                             FUN.VALUE = logical(1))]
        all.count <- ifelse(rowSums(outcomes) > 1L, NA, counts)
        all.count <- all.count * outcomes
        for (j in seq_len(ncol(all.count))) {
            ylist <- list()
            for (i in seq_len(ncontrasts)) {
                ylist[[i]] <- 
    ...
    switch(feature,
           geneModel = {
               ...
               if (length(exon)) {
                   ...
                   tids <- rep(names(cds), elementNROWS(cds))
                   cds <- unlist(cds)
                   if (length(cds)) {
                       mcols(cds) <- NULL
                       cds$tx_name <- tids
                   ...
                   utr5 <- fiveUTRsByTranscript(ranges, use.names = TRUE)
                   tids <- rep(names(utr5), elementNROWS(utr5))
                   utr5 <- unlist(utr5)
                   if (length(utr5)) {
                       mcols(utr5) <- NULL
                       utr5$tx_name <- tids
                   ...
                   utr3 <- threeUTRsByTranscript(ranges, use.names = TRUE)
                   tids <- rep(names(utr3), elementNROWS(utr3))
                   utr3 <- unlist(utr3)
                   if (length(utr3)) {
                       mcols(utr3) <- NULL
                       utr3$tx_name <- tids
                   ...
                   mcols(left) <- mcols(left)[, c("tx_name", "feature_type")]
                   ...
                   if (!missing(OrganismDb)) {
                       if (inherits(OrganismDb, c("OrganismDb"))) {
                           ...
                           if (length(symbol) > 0) {
                               exon$symbol <- 
                                   symbol[match(exon$tx_name, symbol[, 1]),
                                         "SYMBOL"]
                           }
                       } else {
                   ...
                   tids$values <- tapply(start(exon), exon$tx_name, min)
                   tids <- inverse.rle(tids)
                   exon <- exon[order(as.character(seqnames(exon)),
                                      tids, 
                                      start(exon))]
                   names(exon) <- formatC(seq_along(exon), 
           ...
           exon = {
               e <- exons(ranges, 
                         columns = c("exon_id", "tx_name", "gene_id"))
               if (length(e)) {
                   names(e) <- e$exon_id
                   e$exon_id <- NULL
               }
           ...
           CDS = {
               c <- cds(ranges, 
                       columns = c("cds_id", "tx_name", "gene_id"))
               if (length(c)) {
                   names(c) <- c$cds_id
                   c$cds_id <- NULL
               }
           ...
           fiveUTR = {
               u <- fiveUTRsByTranscript(ranges, use.name = TRUE)
           ...
           threeUTR = {
               u <- threeUTRsByTranscript(ranges, use.name = TRUE)
               tids <- rep(names(u), elementNROWS(u))
               u <- unlist(u)
               if (length(u)) {
                   u$tx_name <- tids
...
EnsDb2GR <- function(ranges, feature) {
    gr <- switch(feature,
                 ...
                 gene = {
                     g <- genes(ranges, columns = c("gene_id", "gene_name"))
                     names(g) <- g$gene_id
                     g$gene_id <- NULL
                     g
                 },
...
removeAncestor <- function(goenrichments, onto, cutoffPvalue) {
    stopifnot(colnames(goenrichments)[1] %in% c("go.id")) 
    ## confirm the result is from getEnrichedGO
...
filterByLevel <- function(goenrichments, onto, level = 4) {
    if (level < 2) {
        warning("keepByLevel must be a number larger than 1. ",
